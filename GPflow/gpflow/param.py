    ...
    *Scoping*
    ...
    def __getattribute__(self, key):
        ...
        if isinstance(o, (Param, DataHolder)):
            return o._tf_array
        elif isinstance(o, dict): # + for dictionary
            flag = False
            for val in o.values():
                if isinstance(val, (Param, DataHolder)):
                    pass
                else:
                    flag = True
                    break
    ...
    def sorted_params(self):
        ...
        # + for dictionary (MECGP)
        params = []
        for key, child in self.__dict__.items():
            if isinstance(child, (Param, Parameterized)) and key is not '_parent':
                params.append(child)
    ...
    def _begin_tf_mode(self):
        ...
        for child in self.sorted_params:
            if isinstance(child, Parameterized):
                ...
                flag = False
                for val in child.values():
                    if isinstance(val, Parameterized):
                        pass
                    else:
                        flag = True
                        break
