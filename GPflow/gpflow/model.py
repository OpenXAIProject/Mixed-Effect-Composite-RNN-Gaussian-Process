...
from .feature_representations import Identity
import sys
...
class Model(Parameterized):
    ...
    def __init__(self, name='model'):
        ...
        self._opt_step = None # Do not recompile unless any change happened
    ...
    def _optimize_tf(self, method, callback, maxiter):
        ...
        """
        if self._needs_recompile: # modified to externally optimize iteratively
            self._opt_step = self.compile(optimizer=method)
        feed_dict = {}
        ...
        try:
            ...
            while iteration < maxiter:
                self.update_feed_dict(self._feed_dict_keys, feed_dict)
                _, f_val = self.session.run([self._opt_step, self._minusF], feed_dict=feed_dict)
                self.num_fevals += 1
...
class GPModel_MECGP(Model):
    """
    GPModel for MECGP, following structure of GPModel.
    """

    def __init__(self, X, Y, kern, likelihood, mean_function, feature_representation, p_id, size, name='model'):
        Model.__init__(self, name)
        self.mean_function = mean_function or Zero()
    ...
    def predict_y(self, Xnew):
        """
        Compute the mean and variance of held-out data at the points Xnew.
        Given multi-task patient data as a dictionry of Xnew.
        Return a ditionary type of argument like Xnew.
        """
        #dict Xnew to np array
        stacked_Xnew = []
        for p in self.p_id:
            stacked_Xnew.append(Xnew[p])
        stacked_Xnew = np.concatenate(stacked_Xnew, axis=0)

        @AutoFlow((float_type, [None, None]))
        def _predict_y(self, _Xnew):
            pred_f_mean, pred_f_var = self.build_predict(_Xnew)
            pym_toStack, pyv_toStack = [], []
            size = 0
            for p in self.p_id:
                m, v = self.likelihood.predict_mean_and_var(pred_f_mean[size:size+self.size[p],:], 
                                                            pred_f_var[size:size+self.size[p],:])
                pym_toStack.append(m)
                pyv_toStack.append(v)
                size += self.size[p]
            return tf.concat(pym_toStack, axis=0), tf.concat(pyv_toStack, axis=0)
        
        #np array to dict
        stacked_Yhat, stacked_Ystd = _predict_y(self, stacked_Xnew)
        Yhat, Ystd = {}, {}
        size = 0
        for p in self.p_id:
            Yhat[p] = stacked_Yhat[size:size+self.size[p],:]
            Ystd[p] = stacked_Ystd[size:size+self.size[p],:]
            size += self.size[p]
        return Yhat, Ystd
